...
BEGIN_MESSAGE_MAP(CEasyToonView, CScrollView)
	...
	ON_COMMAND_RANGE(ID_BRUSH_PT0, ID_BRUSH_PT16, OnBrushBtn)
	ON_UPDATE_COMMAND_UI_RANGE(ID_BRUSH_PT0, ID_BRUSH_PT16, OnUpdateBrushBtn)
	...
	ON_WM_KEYDOWN()
	ON_WM_MBUTTONDOWN()
	ON_UPDATE_COMMAND_UI_RANGE(ID_MEM1, ID_MEM4, OnUpdateMemoryBtn)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
CEasyToonView::~CEasyToonView()
	...
	if ( m_UndoRedo )   delete[] m_UndoRedo;
...
void CEasyToonView::OnInitialUpdate() 
	...
	pMainFrame->InitializeMemoryViewer();
	...
	for (int i=0;i<100;i++)
		{
		m_UndoRedo[i].CreateBitmap( m_sizeFrame.cx, m_sizeFrame.cy, 1,  1, pBmpSrc );
		}


	//m_pbmpRedoBuf[UndoRedoPos].CreateBitmap( m_sizeFrame.cx, m_sizeFrame.cy, 1,  1, pBmpSrc );
	...
	m_dcUndoBuf.  SelectObject( m_UndoRedo[UndoRedoPos] );
...
void CEasyToonView::_DrawBackFrame()
	...
	int a_TracePrevFrameNb = 0;
	int a_TraceNextFrameNb = 0;

	a_TracePrevFrameNb = pDoc->GetPrevFrameNb();
	a_TraceNextFrameNb = pDoc->GetNextFrameNb();

	bool firstLoop=true;

	//-------- It is in thin blue about a front frame.
	if ( m_nTraceDepth && (a_TracePrevFrameNb!=0) && (m_nCurrentFrame != 0) && (m_bDrawPrev != false) ) 
		{
		int NbBckFrame = 0;

		if(m_nCurrentFrame-a_TracePrevFrameNb>=0 ) 
			{
			NbBckFrame=a_TracePrevFrameNb;
			}
		else
			{
			NbBckFrame=m_nCurrentFrame;
			}



		for(int i=m_nCurrentFrame-1; i>=m_nCurrentFrame-NbBckFrame;i--)
			{

			int nCont = 256 - (int)(256.0 * m_dTraceFade);
			COLORREF Ref = RGB(nCont*2,nCont*2,nCont);
			m_dcBackFrame.SetTextColor( RGB(nCont*2,nCont*2,nCont) );
			m_dcBackFrame.BitBlt( 0, 0, m_sizeFrame.cx, m_sizeFrame.cy, 
				pDoc->GetFrameDC(i), 0, 0, dwROP );
	...
	if( (m_bDrawNext != false) && (a_TraceNextFrameNb!=0) &&  (m_nCurrentFrame < (pDoc->GetCount()-1) ) ) 
		{
		int NbBckFrame = 0;

		if(m_nCurrentFrame+a_TraceNextFrameNb<=pDoc->GetCount()-1) 
			{
			NbBckFrame=a_TraceNextFrameNb;
			}
		else
			{
			NbBckFrame=pDoc->GetCount() - m_nCurrentFrame;
			}
		...
		for(int i=m_nCurrentFrame+1; i<=m_nCurrentFrame+NbBckFrame;i++)
			...
			m_dcBackFrame.BitBlt( 0, 0, m_sizeFrame.cx, m_sizeFrame.cy, pDoc->GetFrameDC(i), 0, 0, dwROP );
	...
	switch ( m_nTool )
		...
		case pen:
			...
			if(m_nBrush>0 && crPen!=RGB(255,255,255))
				...
				logBrush.lbColor = Couleur::GetColorFromBrushNb(m_nBrush);
		...
		case fill:
			...
			UpdateUndoBuffer( restoreBitmap );
			// Painting out
			CBrush brush(Couleur::GetColorFromBrushNb(m_nBrush) );
...
void CEasyToonView::_EndLineStrip()
	...
	CPen*	ppenOld;
	CRect rectUpdate;
	int oldnInterfacePenSize = 0;
	// Line drawing
	if(m_nBrush>0 && m_crInterfacePenColor!= RGB(255,255,255))
		{

		LOGBRUSH logBrush;
		logBrush.lbStyle = BS_SOLID;
		logBrush.lbColor = Couleur::GetColorFromBrushNb(m_nBrush);
		pen.CreatePen(PS_GEOMETRIC, m_nInterfacePenSize, &logBrush);
		}
	else
		{
		pen.CreatePen( PS_SOLID, m_nInterfacePenSize, m_crInterfacePenColor );
	...
	oldnInterfacePenSize = m_nInterfacePenSize;
	...
	m_ptFrom= m_ptTo;
	m_nInterfacePenSize = oldnInterfacePenSize;
	}
...
void CEasyToonView::EndDraw ( CPoint point, int nBtn )
	...
	switch ( m_nTool )
		...
		case line:case paraline:case linestrip:
			{
			CPen	pen;
			CPen*	ppenOld;
			// Renewal of a UNDO buffer
			...
			if(m_nBrush>0 && crPen!=RGB(255,255,255))
				...
				logBrush.lbStyle = BS_SOLID;
				logBrush.lbColor = Couleur::GetColorFromBrushNb(m_nBrush);
				pen.CreatePen(PS_GEOMETRIC, nPenSize, &logBrush);
				}
			else
				{
...
void CEasyToonView::OnMouseMove(UINT nFlags, CPoint point) 
	...
	CPoint LocalPoint;
	ShowCursor(false);
	SetCursor(AfxGetApp()->LoadCursorA(IDC_PENCURSOR));
	ShowCursor(true);
...
void CEasyToonView::OnFrameDelete() 
	{
	// Šm”F
	if ( MessageBox("Etes-vous sur de vouloir supprimer la frame ?","Attention",MB_OKCANCEL) != IDOK ) return;
...
void CEasyToonView::OnFrameClear() 
	{
	// Šm”F
	if ( MessageBox("Etes-vous sur de vouloir effacer la frame ?","Attention",MB_OKCANCEL) != IDOK ) return;
...
void CEasyToonView::_ClipboardToCopyBuffer()
	...
	try {
		...
		::GlobalUnlock( handle );
			...
			switch( nError ) {
	case 1: MessageBox( "Erreur à l'ouverture." ); break;
	case 2: MessageBox( "Erreur dans l'acquisition des données." ); break;
	case 3: MessageBox( "Ce n'est pas un DIB 24 bits." ); break;
	default: MessageBox( "Erreur" ); break;
				}
		}
	CloseClipboard ();
...
void CEasyToonView::_CopyBufferToClipboard()
	{
	// ‹ó
	if( m_rectCopyDst.IsRectEmpty() != false ) return;
	//  rect.NormalizeRect();

	// ƒRƒs[Œ³‚ÌƒsƒNƒZƒ‹‚ðŠi”[‚·‚é—Ìˆæ‚ðì¬
	BYTE *pSrc;
	int nbSrcBytes = (m_rectCopyDst.Width() >> 3) + (m_rectCopyDst.Width() & 0x7 ? 1 : 0); // ƒ‰ƒXƒ^ƒTƒCƒY
	nbSrcBytes += (2-(nbSrcBytes & 0x1)) & 0x1; // WORD ƒoƒEƒ“ƒ_ƒŠ
	nbSrcBytes *= m_rectCopyDst.Height();
	pSrc = new BYTE[nbSrcBytes];
	// ƒRƒs[Œ³ƒsƒNƒZƒ‹Žæ“¾
	if (m_pbmpCopySrc==NULL)
		_CurrentFrameInterfaceToCopyBuffer(m_rectCopyDst);

	m_pbmpCopySrc->GetBitmapBits( nbSrcBytes, (LPVOID) pSrc );

	// ƒRƒs[æ‚ÌƒTƒCƒY
	int nbDstBytes = m_rectCopyDst.Width()*3; // ƒ‰ƒXƒ^ƒTƒCƒY
	nbDstBytes += (4-(nbDstBytes & 0x3)) & 0x3; // DWORD ƒoƒEƒ“ƒ_ƒŠ
	nbDstBytes *= m_rectCopyDst.Height();
	...
	BITMAPINFOHEADER *pBmpInfoHeader = (BITMAPINFOHEADER*) dst;
	pBmpInfoHeader->biSize = sizeof(BITMAPINFOHEADER);
	pBmpInfoHeader->biWidth  = m_rectCopyDst.Width();
	pBmpInfoHeader->biHeight = m_rectCopyDst.Height();
	pBmpInfoHeader->biPlanes = 1;
	pBmpInfoHeader->biBitCount = 24;
	pBmpInfoHeader->biCompression = BI_RGB;    // –³ˆ³k
	pBmpInfoHeader->biSizeImage = nbDstBytes;
	...
	// ƒsƒNƒZƒ‹‘‚«ž‚Ý
	int x, y;
	for( y = 0 ; y < m_rectCopyDst.Height() ; y++ ) {
		for( x = 0 ; x < m_rectCopyDst.Width() ; x++ ) {
			if( CEasyToonDoc::_GetPixelColor1( pSrc, m_rectCopyDst.Width(), m_rectCopyDst.Height(), x, y ) == 1 ) {
				CEasyToonDoc::_SetPixelColor24( dst + sizeof(BITMAPINFOHEADER), m_rectCopyDst.Width(), m_rectCopyDst.Height(),
...
void CEasyToonView::_CurrentFrameInterfaceToCopyBuffer(CRect rectCopySrc)
	{
	m_dcCopySrc.RestoreDC( -1 );
	...
	if( m_pbmpCopySrc != NULL ) {
		m_dcCopySrc.SelectObject( m_pbmpCopySrc );
		m_dcCopySrc.BitBlt( 0, 0, rectCopySrc.Width(), rectCopySrc.Height(),
			&m_dcInterface, rectCopySrc.left, rectCopySrc.top, SRCCOPY );
		}
	}
...
void CEasyToonView::OnEditUndo() 
	...
	switch ( m_nUndoType )
		...
		case restoreBitmap:		// ƒrƒbƒgƒ}ƒbƒv•œ‹A
			// ƒŠƒhƒDƒf[ƒ^‚ÌXV
			m_dcUndoBuf.SelectObject( m_UndoRedo[UndoRedoPos+1] );
			m_dcUndoBuf.BitBlt( 0, 0, m_sizeFrame.cx, m_sizeFrame.cy, GetDocument()->GetFrameDC(m_nCurrentFrame), 0, 0, SRCCOPY );
			// •œ‹A
			m_dcUndoBuf.SelectObject( m_UndoRedo[UndoRedoPos] );
			GetDocument()->GetFrameDC(m_nCurrentFrame)->BitBlt( 0, 0, m_sizeFrame.cx, m_sizeFrame.cy, &m_dcUndoBuf, 0, 0, SRCCOPY );
...
void CEasyToonView::OnUpdateMemoryBtn ( CCmdUI* pCmdUI )
	{

	}

void CEasyToonView::OnMemoryBtn(bool Saving, int Pos)
	{
	if (!Saving)
		{
		...
		m_dcCopySrc.SelectObject(GetDocument()->GetBitmapFromMemoryList(Pos));
		GetDocument()->GetFrameDC(m_nCurrentFrame)->BitBlt( 0, 0, m_sizeFrame.cx, m_sizeFrame.cy, &m_dcCopySrc, 0, 0, SRCAND );
		...
		{
		GetDocument()->CopyFrameToMemory(m_nCurrentFrame);
		GetMainFrame()->UpdateMemoryViewer();
		}
	}
...
void CEasyToonView::OnFileImportgif() 
	...
	if ( fileDlg.DoModal() == IDOK ) 
		{
		CFile* File = new CFile(fileDlg.GetPathName(),CFile::OpenFlags::modeRead); 
		CArchive* ar = new CArchive(File,CArchive::Mode::load);
		GetDocument()->ImportGif(ar,false, m_nCurrentFrame);
...
void CEasyToonView::_DrawSelectionRect( CDC *pDC, CRect rect, int nPick)
	...
	// ƒyƒ“ì¬
	CPen pen( PS_SOLID, 1, RGB(0,255,0)), *ppenOld;
	ppenOld = pDC->SelectObject( &pen );
	...
	if (!m_rectCopySrc.IsRectEmpty())
		...
		switch(type)
			...
			case Vertical:
				...
				dcTemp.StretchBlt(0,0,m_sizeFrame.cx,m_sizeFrame.cy,GetDocument()->GetFrameDC(m_nCurrentFrame),0,m_sizeFrame.cy-1,m_sizeFrame.cx,-m_sizeFrame.cy,SRCCOPY);

				break;

			case Horizontal:

				dcTemp.StretchBlt(0,0,m_sizeFrame.cx,m_sizeFrame.cy,GetDocument()->GetFrameDC(m_nCurrentFrame),m_sizeFrame.cx-1,0,-m_sizeFrame.cx,m_sizeFrame.cy,SRCCOPY);
				break;
			}
...
void CEasyToonView::OnText()
	...
	lf.lfWeight=DlgFont.GetWeight();
	strcpy_s(lf.lfFaceName, DlgFont.GetFaceName());        // request a face name "Arial"
...
void CEasyToonView::DrawBackground()
	{
	_DrawBackFrame();
	Invalidate(false);
	}
