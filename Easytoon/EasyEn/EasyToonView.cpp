...
{
	// Šm”F
	if ( MessageBox("Are you sure you want to delete this frame","Delete frame",MB_OKCANCEL) != IDOK ) return;
...
{
	// Šm”F
	if ( MessageBox("Are you sure you want to clear this frame","Clear frame",MB_OKCANCEL) != IDOK ) return;
...
void CEasyToonView::_ClipboardToCopyBuffer()
{
	...
	try {
		...
		switch( nError ) {
	case 1: MessageBox( "Failed to open" ); break;
	case 2: MessageBox( "Failed to acquire data" ); break;
	case 3: MessageBox( "This is not a 24 bits DIB" ); break;
	default: MessageBox( "Something error" ); break;
		}
	}
	CloseClipboard ();
